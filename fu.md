Да. Ниже полный скрипт. Логика категорий не меняется: `pk2_ot1` остаётся общей бизнес-категорией, но дополнительно появляются отдельные объёмы и флаги `pk2` и `ot1`.

Если у одного договора одновременно `Пк2=1` и `От1=1`, внутри общей категории он относится к `PK2`, поэтому всегда выполняется:
...
BEGIN
    ...
    SET @LoopEndDate =
        EOMONTH(DATEADD(month, 1, @LoopBaseDate));
...
/* ============================================================
   ...
   Пк2 и От1 теперь храним как:
   - отдельные признаки;
   - общий признак pk2_ot1 для старой бизнес-логики.
   ============================================================ */
...
SELECT
    ...
    /* Нужны дальше для внутреннего разделения */
    , e.is_pk2_flag
    , e.is_ot1_flag


    /* Общая бизнес-классификация НЕ меняется */
    , CASE
          WHEN e.is_fu_flag = 1
              THEN N'fu'
...
/* ============================================================
   ...
   Когорта по-прежнему строится по общей категории Пк2 / От1.
   ============================================================ */
...
/* ============================================================
   7. КЛИЕНТСКИЕ ФЛАГИ PREMIUM / SALARY
   ============================================================ */
...
SELECT
    ...
    /* Нужны для отдельного PK2 / OT1 */
    , o.is_pk2_flag
    , o.is_ot1_flag


    /* Общая классификация остаётся старой */
    , CASE
          WHEN o.is_fu_flag = 1
              THEN N'fu'
...
(
    SELECT
        ...
        /* ====================================================
           Пк2 / От1 — ОБЩАЯ старая категория
           ==================================================== */
        ...
        /* ====================================================
           ...
           Если одновременно Пк2=1 и От1=1,
           договор относим сюда.
           ==================================================== */

        , SUM(
              CASE
                  WHEN e.exit_category = N'pk2_ot1'
                       AND e.is_pk2_flag = 1
                      THEN e.out_rub
                  ELSE 0
              END
        ...
        /* ====================================================
           ...
           Только если Пк2 одновременно НЕ стоит.
           ==================================================== */
...
(
    SELECT
        ...
        , SUM(
              CASE
                  WHEN o.open_category = N'pk2_ot1'
                       AND o.is_pk2_flag = 1
                      THEN o.out_rub
                  ELSE 0
              END
...
Теперь в `#client_mart` одновременно есть старые бизнес-поля:
...
При этом когорта по-прежнему определяется по общей категории `pk2_ot1`, поэтому сам винтажный анализ не меняется.
